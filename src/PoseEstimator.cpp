 ...
 * @return
 */
std::vector<cv::Mat_<double>> PoseEstimator::solvePoseBy68Points(std::vector<cv::Point2f> imagePoints){
...
void PoseEstimator::getFullModelPoints(){
    ...
    for(int i = 0; i < 68; i++) {
        cv::Point3d point = cv::Point3f(rawValue[i], rawValue[i + 68], rawValue[i + 136] * -1);
...
std::vector<double> PoseEstimator::convertToEularAngle() {
...
std::vector<cv::Point> convert2I(std::vector<cv::Point2d> inPoints){
...
//    //投影函数
//    cv::projectPoints(point3d, this->old_rVec, this->old_tVec, cameraMatrix, cv::noArray(), imagePoints);

//    outPoints = convert2I(imagePoints);

//    //绘图
//    cv::polylines(image, outPoints, true, cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
//    cv::line(image, outPoints[1], outPoints[6], cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
//    cv::line(image, outPoints[2], outPoints[7], cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
//    cv::line(image, outPoints[3], outPoints[8], cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
}
