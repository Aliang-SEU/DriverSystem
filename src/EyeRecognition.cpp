...
EyeRecognition::EyeRecognition(float fps){
    ...
    this->minLength = static_cast<int>(fps * 2);
    this->halfLength = static_cast<int>(fps * 10);
    this->maxLength = static_cast<int>(fps * 30); //这里设置30s为一个基准长度
}
...
bool EyeRecognition::decideFatigue(int length, float ratio) {
    ...
    for(int i = 0; i < length; i++) {
        if(eyeStateQueue.at(static_cast<size_t>(length - i - 1)) == CLOSED) {
            closeCount++;
        }
    }
    int thresh = static_cast<int>(ratio * length);
 ...
 * @param rightEye
 */
bool EyeRecognition::recordEyeState(enum EyeState leftEye, enum EyeState rightEye){
...
//    }
    ...
    //短时间检测
    if(static_cast<int>(eyeStateQueue.size()) > minLength) {
        bool flag = decideFatigue(minLength, 0.8f);
        if(flag == true) {
            eyeStateQueue.clear();
            return true;
        }
